...
from . import critical, machine, polygons  # multigrid solver
...
class Equilibrium():
    ...
    def plasmaBr(self, R, Z):
        ...
        return -self.psi_func(R, Z, dy=1) / R
    ...
    def plasmaBz(self, R, Z):
        ...
        return self.psi_func(R, Z, dx=1) / R
    ...
    def psi_1D(self, N):
        ...
        return jnp.linspace(self.psi_axis, self.psi_bndry, N)
    ...
    def separatrix(self, ntheta=360, IQR_factor=2.0, verbose=False):
        ...
        points = jnp.array(critical.find_separatrix(self, ntheta=ntheta))[
            :, 0:2
        ]
    ...
    def innerOuterSeparatrix(
        ...
        Zindex = jnp.argmin(abs(self.Z[0, :] - Z))
        ...
        Rindex_axis = jnp.argmin(abs(self.R[:, 0] - self.Rmagnetic()))
    ...
    def effectiveElongation(self):
        ...
        return self.plasmaVolume() / (2.0 * R_geom * (jnp.pi**2) * (R_minor**2))
    ...
    def internalInductance1(self):
        ...
        # volume elements
        dV = 2.0 * jnp.pi * self.R * self.dR * self.dZ
        ...
        integral = jnp.sum(self.Bpol(self.R, self.Z) * dV)
    ...
    def internalInductance2(self):
        ...
        integral_Bpol_2 = jnp.sum((self.Bpol(self.R, self.Z) ** 2) * dV)
        ...
        separatrix = self.separatrix()
        dl = jnp.sqrt(
            jnp.diff(separatrix[:, 0]) ** 2 + jnp.diff(separatrix[:, 1]) ** 2
        )  # element-wise arc length
        l = jnp.concatenate(([0], jnp.cumsum(dl)))  # cumulative arc length

        # evaluate Bp on the LCFS
        Bp = self.Bpol(separatrix[:, 0], separatrix[:, 1])

        # integrate
        integral_Bpol_lcfs = jnp.trapz(Bp, l)
    ...
    def internalInductance3(self):
        ...
        # integrate B_pol squared over the volume
        integral = jnp.sum((self.Bpol(self.R, self.Z) ** 2) * dV)
    ...
    def poloidalBeta2(self):
        ...
        field_integral_pol = jnp.sum((self.Bpol(self.R, self.Z) ** 2) * dV)
    ...
    def poloidalBeta4(self):
        ...
        R_avg = jnp.sum(self.R * dV) / self.plasmaVolume()
    ...
    def toroidalBeta1(self):
        ...
        pressure_integral = jnp.sum(pressure * self.dR * self.dZ)
        Btor2_integral = jnp.sum(Btor2 * self.dR * self.dZ)
    ...
    def toroidalBeta2(self):
        ...
        Btor2_int = jnp.sum((self.Btor(self.R, self.Z) ** 2) * dV)
